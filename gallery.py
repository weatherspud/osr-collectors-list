...
IMAGE_PREFIX_FMT = 'https://raw.githubusercontent.com/weatherspud/osr-collectors-list/master/{subdir}'
    ...
    try:
        with open(checklist_path, encoding='utf-8') as f:
            ...
            for i, row in enumerate(rows[1:]):
                ...
                title = row[header.index(HEADER_TITLE)]
                year = row[header.index(HEADER_YEAR)]
                width_pct = int(100.0 / args.items_per_row)
                print(f'<td valign="top" width="{width_pct}%"><img src="{image_prefix}/{image}" height="240"><br/><br/>{title}<br/>{year}</td>')
